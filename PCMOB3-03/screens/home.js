...
export default function homeScreen( {navigation, route} )
{
  ...
  useEffect(() => {
    navigation.setOptions({
      headerRight: () => 
        (
          <TouchableOpacity onPress={() => addNote("new", null)}>
              <Ionicons
  ...
  function toDoList( {item} )
  {
    return (
        <View style={styles.toDoListItemContainer}>
            <TouchableOpacity onPress= {() => addNote("modify",item.id)}>
            ...
            <View style={styles.toDoListIconsContainer}>
              ...
              : null }
              { item.done === 1 ? 
              <TouchableOpacity onPress= {() =>uncheckItem(item.id)}>
                  <Ionicons
  ...
  return (
    <View style={styles.container}>
      <FlatList style = {styles.toDoList} data = {notes} renderItem = {toDoList}/>
    </View>
  );
}
